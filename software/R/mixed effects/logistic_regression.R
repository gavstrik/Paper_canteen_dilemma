...
predict(m2)
Data$pred = plogis(predict(m2)) # det er sandsynlighederne for at gå i kantinen for hver spiller
tail(Data)
...
ggplot(data=predicted.data, aes(x=arr, y=probability.of.choosing.canteen, colour = exp)) + 
  geom_point() +
  geom_smooth(formula = choice ~ arr*exp, method = "glm", method.args = list(family = "binomial")) +
  xlab("Arrival") + 
