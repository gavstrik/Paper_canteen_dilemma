...
A fixed participation fee of \$2 is given to all MTurk players. Additional bonuses are calculated as penalties substracted from the initial endowment of \$10 (MTurk) or \$30 (DTU) in each round. Payoffs are tiered in such a way that a small penalty is deducted for successful coordination into the canteen (achieving the highest payoff), which is doubled for coordination into the offices (achieving the second-highest payoff), while the penalty for miscoordination or forbidden choices, i.e.\ going to the canteen at 9 am or after, is much larger (up to 921 times larger, meaning a significantly lower payoff than the previous two). This means that if a pair of players miscoordinate often, they risk loosing all their endowment, in which case their game terminates prematurely. Penalties are calculated by a logarithmic scoring rule, which depends on the decision and the certainty estimate by each player. Using penalties instead of bonuses may seem an unintuitive way to reward participants, but in the literature of scoring rules\tobo{What does literature *of* scoring rules mean?}, logarithmic scoring rules are common and known to work well, due to their ability to ensure that loss minimization remains central, and that the player's actions represent their actual beliefs~\citep{good1992rational, seidenfeld1985calibration, palfrey2009eliciting, mccutcheon2019favor}. In addition, we note that logarithmic scoring is a ``strictly propper scoring rule'' and generally does not create a cognitive overload \tobo{shouldn't there be a citation here?}. As noted in Appendix~\ref{appendix:payoffs}, we accordingly find a good match between certainty estimates and choices at arrival times different from those that are prone to miscoordination. This corroborates that players tried to minimize their losses and that they made their choices and certainty estimates as honestly as possible. 
...
Second, we can ask if situations arise in the wild where there is only shared knowledge to some degree but not common knowledge---and where we would have a tendency to assume common knowledge with potentially large negative consequences. We will argue that such situations are quite common. 
% the lack of common knowledge can have 
%. It is important to note here that the essential problem in both the mountaineering example and our canteen dilemma is the lack of reliable signalling. 
%And unlike those cases, we can assume in most real cases that our signal or message (both face-to-face and online) has reached the recipient.  But a signal reliably reaching someone is not sufficient for establishing common knowledge about its message. They must also reliably interpret it \emph{and} this reliableness must be common knowledge.  So, depending on the context and complexity of the intentionality involved, I might not be sure that my communicated intention becomes common knowledge. Note that this is because that humans do not just lack logical omniscience, often prescribed in logical analysis, but also perfect observation and recall. That is, if we make an announcement of something, we cannot be certain that everyone comes to know this, or that they will maintain it as knowledge. 
Public broadcasting of messages, e.g.\ instructions given in a classroom to a class of pupils or an email sent by the department head to everybody in a university department, will in practice not necessarily lead to common knowledge. Some of the pupils might not be paying attention at the moment, and some of the department employees might not have checked their email, or it might have ended up in their spam filter. Suppose that the message was regarding to all meet at a certain place at a certain time. Can we then be sure that everyone will meet at that place at that time? Obviously, no. Then the reply might be that we can just ask everybody to confirm that they received the message, e.g.\ the school teacher might ask the children to confirm that they heard the message. Well, yes, but then the same argument repeats for the confirmation messages: the recipient of the confirmations might not receive all of them or might briefly not pay attention. Now, this generalizes to all social situations and while it does not mean that we do not often obtain common knowledge, it means that we sometimes do not, and end in epistemic situations akin to the canteen dilemma. No amount of confirmations can guarantee common knowledge, and the risk of miscoordination will still be there. However, quite likely, no matter how large the penalty is for miscoordination, when the level of shared knowledge has become sufficiently high, we will act as if common knowledge has already been achieved. %And probably in most real-life cases the penalty for falsely assuming common knowledge might be somewhat lower than in the canteen dilemma, e.g.\ it might just be that someone doesn't show up for the event that the original message concerned. 
At least, this is our hypothesis, and it is at least being confirmed in the setting of the canteen dilemma.  

In most everyday situations, the penalty for confusing shared knowledge with common knowledge is probably much lower than in our game. If it is for instance a group of pupils who are asked to meet at a certain time and place to get on the bus home from a school trip, confusing shared knowledge with common knowledge is not likely to be a big problem. It just means that the children will meet at the designated time and place despite not having common knowledge that all other children will also be there. In this case, the confusion of shared knowledge with common knowledge might even be helpful, since it allows coordination despite the lack of common knowledge. But this only happens because there is no large penalty if coordination fails. Even if one child eventually doesn't show up, it is probably better for all the other children to arrive anyway rather than not going due to the risk of someone else not showing up. In certain cases, it could however be different.
If a group tries to coordinate a public demonstration, a dangerous emergency rescue or coordinate to all write letters of complaint, they might individually suffer a high penalty if some or all other group members end up not participating. In this case, confusing shared knowledge with common knowledge can be critical, and it can be critical to be in a situation where common knowledge can not be achieved (e.g.\ if trying to coordinate via posts on social media or trying to shout through the walls of a burning building).  
...
Still, probably in most real-world scenarios, the penalty for miscoordination is lower than in our canteen dilemma. However, if that is indeed the case, then we should expect humans to pay \emph{more} attention to the potential confusion of shared knowledge for common knowledge in the game than in the wild. 
%However, if the penalty of confusing shared knowledge for common knowledge is most often lower in the wild than in our game, it should lead humans to pay more attention to it in our game than in the wild.
 In other words, if participants do not pay attention to it in our game, we should not expect them to be more aware of it in the wild.
We might indeed have evolved to do our best to avoid such situations, due to the complexity involved. This might explain why we do not identify it everywhere in our daily life. But it is unlikely that we can categorically deny that such situations arise, and when they do, some might implicitly assume common knowledge to be present, while others, on a lower order of shared knowledge might not. This results in miscoordination which people cannot predict, and possibly not even post-hoc understand. 
...
We believe that our results may also be of practical interest for legislative efforts trying to create social rules of conduct in situations where coordination without explicit communication is common. As an example, we can mention the recent changes in global legislations on sexual assault: within the last 20 years, more than 30 countries globally have shifted or are in the process of shifting from a coercion-based model, which requires that for a sexual act to be considered rape there needs to be proof of coercion, to a consent-based model, which stipulates that sex without mutual consent is to be considered as rape. The main reason for this change is that sexual assault often occurs in situations where friends or partners miscoordinate their signals and become unable and/or unwilling to change their behavior~\citep{mcgregor2017rape, amnesty2018yes, koljonen2019how, wikipedia2021sexual}. 

In other words, sexual consent law frequently deals with ambiguous coordination problems where private or shared knowledge has been wrongly interpreted as being common knowledge. Imagine the following real world scenario: instead of Agnes and Bertram meeting at the top of a mountain or for a cup of coffee in the canteen, we look at them 
%trying to meet for a cup of coffee, we look at pairs of people 
trying to to interpret certain behavioral signals of each other as either romantic or not ("she/he makes a certain comment", "he/she touches you", etc..), and decide if they want to make a sexual advance or not. The issue is that it might not be enough that the other is interested, certain actions requires consent as well to be permissible, like most touching of a sexual kind. The question then becomes: at which point may a direct sexual advance be permissible, if ever? If only one of them is interested, it is clearly wrong to make a move. If both are interested, but don't know whether the other one is as well, they still ought to refrain from action, as they can't be certain that the other one is interested. Suppose now that it is shared knowledge that both are interested. Then they both individually know that the interest is mutual.
% Then Bertram knows that Agnes is interested.
%each of them knows the other is interested. 
% E (iA /\ iB)  \equiv K_A (iA /\ iB) /\ K_B (iA /\ iB). 
% ~ K_B K_A (iA /\ iB).  
However, if it is only first-order shared knowledge, Agnes doesn't know that Bertram knows that the interest is mutual. Hence, if Bertram makes a sexual advance, Agnes considers it possible that he is making the advance without knowing that the interest is mutual. Hence, Agnes considers it possible that he is making an advance in one of the situations where he clearly shouldn't, which Agnes might very reasonably see as off-putting or at least as raising a red flag. The conclusion is that even first-order shared knowledge is not in general sufficient for it to be appropriate and safe to make a sexual advance. The argument can be repeated to show that even second-order shared knowledge is insufficient: If it is inappropriate for Bertram to make an advance based on first-order shared knowledge, it will also be inappropriate to make an advance based on second-order shared knowledge, as Agnes will then still consider it possible that Bertram is making an advance based on first-order shared knowledge only.     
%he subject of the advance will consider it possible that the one making the advance is only aware of there being first-order shared knowledge.     

%Bertram 
%doesn't know that Agnes knows that  
%don't know that the other person knows you're interested. 
%in this case, each agent is in the same knowledge state as in the case where only they, but not the other person, is interested. 
% as it might be off-putting or seen as a red flag for you, if the other person makes a sexual advance without knowing you were interested (since the person in this case would have done this regardless of your interest in them).
%If both of them are interested, they still only have private knowledge and do not know whether the other is interested as well, and ought to refrain from action
%. In fact, even given mutual interest, it might be off-putting or seen as a red flag for you, if the other person makes a sexual advance without knowing you were interested (since the person in this case would have done this regardless of your interest in them). If the other person has first-order shared knowledge (i.e.\ they know that you are interested), nothing has changed for you, and their advance may still be off-putting, meaning they ought to refrain from action in this case. Now, even if you also know that the other person is interested, the situation hasn't changed from their perspective, and they again refrain from action. 
%Note, it is not a question whether you enjoy or feel harashed from a person actions, which alone determines the permissibility of  some action [actions are only judged purely by consequences in utilitarianism]. If some person performs some action indiscriminately and non consentually towards random  people, most of whom rightfully feel harashed, while few are thankful, the action was wrong in all of the cases, because the person cannot tell the situations apart. That is, the good cases are extensively the same as the good cases from his point of view.


The point, of course, is that this can be generalized in the same way as in our game. As long as there is not established common knowledge about consent in our case, any behavior which tries to make some type of epistemic cut-off, where it is permissible to make a certain move in one situation, but impermissible on a lower epistemic level, will necessarily lead to miscoordination, which in the romantic case amounts to anything from awkwardness to harassment. So, it follows that we must either accept the risk of miscoordination, or stick to inaction until we have achieved common knowledge about mutual consent. Common knowledge about mutual consent can be achieved via explicit agreement, however, traditionally sexual advances has probably been made on less reliable signalling of romantic interest that only would guarantee shared knowledge (or shared belief) to a certain depth.  
%
%Common knowledge about mutual consent \emph{can} of course be achieved in the case of sexual advances, however, it requires the consent to be given explicitly.  
%
 %
% (luckily, this is generally possible in real life, and not too difficult either, unlike in the canteen dilemma).
%they still may see it possible that the other player only has private knowledge (see Fig.~\ref{fig:indistinguish})
The wider implications, supported by our findings, are clear: If we do not take seriously the fact that we often are unable to properly distinguish between common knowledge and shared knowledge, we cannot hope to mitigate the harmful effects arising from it. That is, players in our game might not appreciate the importance of a public announcement that they both arrived before 9:00, but will run into miscoordination without it. Our results therefore show the importance of emphasizing open, honest and reliable communication, even when not intuitively necessary, in situations where it matters. 
...
We let $T$ denote the set of arrival pairs.  
...
We will now try to determine the possible candidates for being Pareto optimal strategy profiles. We do this by iteratively removing strategy profiles that are not Pareto optimal.
...
We want to show that $EU(\mathbf{s'}) > EU(\mathbf{s})$. First note that
\[
 \begin{array}{rll}
 u_{(t,t+10)}({\bf s}')\!\!\!\!\!{ } &= u_{(t,t+10)} (c, s_2(t+10)) &\text{by def.\ of $\bf s'$} \\
 						&= u_{(t,t+10)} (c, c) &\text{by def.\ of $\bf s$} \\
						&> u_{(t,t+10)} (o,c) &\text{by constraint U1, as } t+10 < 9{:}00 \\
...
\end{definition}
Note that the strategy we in the informal discussions above referred to as the ``canteen-before-9'' strategy is the cut-off strategy with cut-off $8{:}55$ (or any other cut-off strictly between $8{:}50$ and $9{:}00$). We now get the result on optimal strategies claimed in the informal discussion.
...
\end{theorem}
\begin{proof}
Let $\bf s$ be a Pareto optimal strategy profile. %Assume that $\tmin$ is of the form $8{:}50-20x$ for some $x \geq 0$ and $\tmax$ is of the form $9{:}00+20y$ for some $y \geq 0$, the other cases being treated symmetrically. 
