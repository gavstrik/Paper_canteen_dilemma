...
Let $s$ be a Pareto optimal strategy. For all arrival times $t$, if $s(t)(o) > 0$ then $s(t+10)(o) > \frac12$.
...
\end{lemma}
\begin{proof}
Let $s$ be a strategy with $s(t)(o) > 0$ and $s(t+10)(o) \leq \frac12$ for some $t$. We need to find a strategy $s'$ Pareto dominating $s$. %We only consider the case $i=1$, the case of $i=2$ being symmetric. 
%Then we have $s_1(t)(o) >0$ and $s_2(t+10)(c) \geq \frac{1}{2}$.  
If $s(t')(c) > 0$ for some $t' \geq 9{:}00$, the existence of a strategy dominating $s$ follows
immediately from Lemma~\ref{lemma:no-canteen-at-nine}. We can hence in the following assume that $s(t')(c) = 0$ for all $t' \geq 9{:}00$.  Since $s(t+10)(o) < 1$, we can thus also conclude that $t + 10 < 9{:}00$.
Now define $s'$ to be identical to $s$ except that we let $s'(t')(c) = 1$ for all $t' \leq t$.
%\[
%  \begin{array}{ll}
...
We want to show that $EU(s') > EU(s)$. First consider the arrival pair $\mathbf{t} = (t,t+10)$. We want to show that $u_{\bf t}(s') > u_{\bf t}(s)$, i.e., that $u_{\bf t}(s') - u_{\bf t}(s) >0 $. Let $p = s(t)(o)$ and $q = s(t+10)(o)$. Note that by assumption, $p > 0$ and $q \leq \frac12$. Then, by definition of expected payoff, $u_{\bf t}(s) = p q u_o  + (p(1-q) + (1-p)q) u_m + (1-p)(1-q)u_c$. In $s'$, we have the same but with $p=0$, so $u_{\bf t}(s') =  q u_m + (1-q)u_c$. We then get $u_{\bf t}(s') - u_{\bf t}(s) = p(2q-1) u_m + p(1-q) u_c - pq u_o = p(1-q) u_c - p(1-2q) u_m - pq u_o$. Since $p>0$, this is positive iff $(1-q) u_c - (1-2q) u_m - q u_o$ is. Since $q \leq \frac12$, we have $1-2q \geq 0$. Since $u_o > u_m$, we then get  $(1-q) u_c - (1-2q) u_m - q u_o \geq (1-q) u_c - (1-2q) u_o - q u_o = (1-q) (u_c - u_o)$. Since $q \leq \frac12$ and $u_c > u_o$, this expression is positive, and hence 
%0.9 uc + 0.8 u_m - 0.9u_o
$u_{\bf t}(s') - u_{\bf t}(s) >0 $, as wanted. %Suppose instead that $q > \frac12$. Then $1-2q < 0$. 
% (1-q) u_c + (2q-1)u_m - q u_o.  q=0.9: 0.1u_c + 0.8u_m - 0.9u_o
% u_c > u_o > u_m. u_c = 1, u_o = 0.9, u_m = 0. Then 0.1*1 - 0.9*0.9 = 
%Since $u_o > u_m$, we get  $p(1-q) u_c - p(1-2q) u_m - pq u_o \geq p(1-q) u_c - p(1-2q) u_o - pq u_o = p(1-q) u_c - p(1-q) u_o = p(1-q) (u_c - u_o)$. Since $p > 0$, $q < 1$ and $u_c > u_o$, we get that the expression is positive, showing $u_{\bf t}(s') - u_{\bf t}(s) > 0$.
% all $o$ choices at $t$ are replaced by $c$ choices, by definition, 
%$c$ is chosen with probability 1 at $t$, by definition, 
%whereas the other player preserves her choice at $t+10$. This means that to compute $u_{\bf t}(s')$ from $u_{\bf t}(s)$, we need to replace each occurrence of $(o,x)$ by $(c,x)$, i.e., $u_{\bf t}(s') = u_{\bf t}(c,o) p q + u_{\bf t}(c,c) p(1-q) + u_{\bf t}(c,o) (1-p)q + u_{\bf t}(c,c) (1-p)(1-q)$. We then get $u_{\bf t}(s') - u_{\bf t}(s) = (u_{\bf t}(c,o) - u_{\bf t}(o,o)) pq + (u_{\bf t}(c,c) - u_{\bf t}(o,c)) p (1-q) = (u_{\bf t}(c,c) - u_{\bf t}(o,c)) p (1-q) -  (u_{\bf t}(o,o) - u_{\bf t}(c,o)) pq$.  From constraint U1, we get that $ u_{\bf t}(c,c) - u_{\bf t}(o,c) > u_{\bf t}(o,o) - u_{\bf t}(c,o)$. Since we have assumed  $s(t+10)(c) \geq \frac12$, we get $q = s(t+10)(o) \leq \frac12$ and hence $p (1-q) \geq pq$. Since we have assumed $s(t)(o) >0$, we also have $p >0$. Hence $(u_{\bf t}(c,c) - u_{\bf t}(o,c)) p (1-q) -  (u_{\bf t}(o,o) - u_{\bf t}(c,o)) pq > 0$, so $u_{\bf t}(s') - u_{\bf t}(s) >0$. 
%First note that at the arrival pair $(t,t+10)$, $\bf s'$ by definition specifies $c$ for both players (with probability 1). By constraint U1, and since $t +10 < 9{:}00$, this choice has a strictly better payoff than any other choice. Since $s_1(t)(o) > 0$, in particular the strategy $\bf s'$ has a better payoff at $(t,t+10)$ than $\bf s$, i.e., $u_{(t,t+10)}({\bf s}') > u_{(t,t+10)}({\bf s})$. 
% $u_{(t,t+10)}({\bf s}') = u_{(t,t+10)} (c, s_2(t+10))$ by definition of $\bf s'$.
%\[
% \begin{array}{rll}
...
If $t'_i \leq t$ then either we also have $t'_{3-i} \leq t$ or else $t'_i = t$ and $t'_{3-i} = t+ 10$. The latter case has already been covered, so we only need to consider the case of $t'_1,t'_2 \leq t$. Then $s'(t'_1)(c) = s'(t'_2)(c) = 1$, by definition of $s'$, so $u_{\bf t'}(s') = u_c$. Since $u_c$ is the highest possible payoff, we immediately get $u_{\bf t'}(s') \geq u_{\bf t'} (s)$, and we're done.
...
\end{lemma}
\begin{proof}
Let $s$ be a strategy with $s(t) = c$ and $s(t+10) = c$ for some $t < 8{:}50$. We need to show that $s$ is not Pareto optimal. We can assume $t$ to be odd, the other case being symmetric. Consider the strategy $s'$ that is like $s$ except $s(t+10)(c) =1$. Note that $u_{\bf t}(s') = u_{\bf t}(s)$ when $\bf t$ doesn't contain $t+10$. To prove that $s'$ dominates $s$, we hence only need to prove that $u_{(t,t+10)}(s') + u_{(t+20,t+10)}(s') - u_{(t,t+10)}(s) - u_{(t+20,t+10)}(s) > 0$. Let $p = s(t+10)(o)$ and $q = s(t+20)(o)$. Note that since $s(t+10)(c) < 1$, we have $p > 0$. 

Consider first the case where $t + 20 < 9{:}00$. %Let $x = u_{(t,t+10)}(c,c)$. Then also $x = u_{(t+20,t+10)}(c,c)$ by constraint U2. Let $y = u_{(t+20,t+10)}(o,c)$. Then $y = u_{(t+20,t+10)}(c,o) = u_{(t,t+10)}(c,o)$ by constraint U1. 
Since $s'(t)(c) = 1$ and $s'(t+10)(c) = 1$ and $s(t+20)(c) = 1-q$, we get $u_{(t,t+10)}(s') = u_c$ and $u_{(t+20,t+10)}(s') = (1-q) u_c + q u_m$ and hence $u_{(t,t+10)}(s') + u_{(t+20,t+10)}(s') = (2-q)u_c + q u_m$. %Let $z =  u_{(t+20,t+10)}(o,o)$. Then 
Similarly, we get $u_{(t,t+10)}(s) = (1-p)u_c + p u_m$ and $u_{(t+20,t+10)}(s) = (1-p)(1-q)u_c + (p(1-q)+ (1-p)q) u_m + pq u_o = (1-p-q+pq)u_c + (p+q-2pq) u_m + pq u_o$, and hence $u_{(t,t+10)}(s) + u_{(t+20,t+10)}(s) = (2-2p-q+pq)u_c + (2p+q-2pq) u_m + pq u_o$. Now we can compute the difference that we wanted to prove to be positive: $u_{(t,t+10)}(s') + u_{(t+20,t+10)}(s') - u_{(t,t+10)}(s) - u_{(t+20,t+10)}(s) = p(2-q) u_c - 2p(1-q) u_m - pq u_o$. Recall that we have $u_c > u_o > u_m$. Since also $p >0$, we get $p(2-q) u_c - 2p(1-q) u_m - pq u_o  > p(2-q) u_c - 2p(1-q) u_c - pq u_c = 0$, as required.  


%Then we get $u_{(t,t+10)}(s') + u_{(t+20,t+10)}(s') - u_{(t,t+10)}(s) - u_{(t+20,t+10)}(s) = u_{(t,t+10)}(c,c) + u_{(t+20,t+10)}(o,c) q + u_{(t+20,t+10)}(c,c) (1-q) - u_{(t,t+10)}(c,o) p - u_{(t,t+10)}(c,c) (1-p) - u_{(t+20,t+10)}(o,o) pq - u_{(t+20,t+10)}(c,o) p(1-q) - u_{(t+20,t+10)}(o,c) (1-p)q - u_{(t+20,t+10)}(c,c) (1-p)(1-q)$. Consider first the case where $t + 20 < 9{:}00$. By constraint U1 we then have $u_{(t,t+10)}(c,c) = u_{(t+20,t+10)}(c,c)$ and $u_{(t+20,t+10)}(o,c) = u_{(t+20,t+10)}(c,o) = u_{(t,t+10)}(c,o)$. The expression from before can hence be reduced to $u_{(t,t+10)}(c,c) p (2- q) + u_{(t,t+10)}(c,o) 2p (q-1) - u_{(t+20,t+10)}(o,o) pq \geq u_{(t,t+10)}(c,c) p (2- q)  - u_{(t+20,t+10)}(o,o) pq > 0$, since $u_{(t,t+10)}(c,c) > u_{(t+20,t+10)}(o,o)$, $p(2-q) \geq pq$ and $p >0$. This shows the required when $t + 20 < 9{:}00$. 


Assume instead $t + 20 \geq 9{:}00$. Since $t < 8{:}50$, we must have $t + 20 = 9{:}00$.
By Lemma~\ref{lemma:no-canteen-at-nine} we have $s(t+20)(o) = 1$, i.e., $q = 1$. 
%As before, let $x = u_{(t,t+10)}(c,c)$ and $y = u_{(t+20,t+10)}(o,c)$. Since $t + 20 \geq 9{:}00$, by constraint U2 we have $y = u_{(t+20,t+10)}(c,c) = u_{(t,t+10)}(c,o) = u_{(t+20,t+10)}(c,o)$. Let $z = u_{(t+20,t+10)}(o,o)$. 
As before we have $u_{(t,t+10)}(s') =  u_c$, but now we get $u_{(t+20,t+10)}(s') = u_m$.  Also as before we have $u_{(t,t+10)}(s) = (1-p)u_c + p u_m$, but now we get $u_{(t+20,t+10)}(s) = (1-p)u_m + p u_o$. Hence  $u_{(t,t+10)}(s') + u_{(t+20,t+10)}(s') - u_{(t,t+10)}(s) - u_{(t+20,t+10)}(s) = u_c + u_m - (1-p)u_c - p u_m - (1-p)u_m - p u_o = p u_c - p u_o$. Since $u_c > u_o$ and $p > 0$, we get $p u_c - p u_o > 0$, as required. 
%u_{(t,t+10)}(c,c) + u_{(t+20,t+10)}(c,o)  - u_{(t,t+10)}(c,o) p - u_{(t,t+10)}(c,c) (1-p) - u_{(t+20,t+10)}(o,o) p  - u_{(t+20,t+10)}(c,o) (1-p) = u_{(t,t+10)}(c,c) p + u_{(t+20,t+10)}(c,o) p - u_{(t,t+10)}(c,o) p - u_{(t+20,t+10)}(o,o)p$.
\end{proof}
