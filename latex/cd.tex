...
A relevant discussion relates to how close our experimental setup is to real world scenarios. We identify a few possible problems here. First, we might ask if participants had a natural preference for canteen, making them choose canteen and resulting in miscoordination. We started out with informal testing on people using integer values instead of arrival times, and asking them to coordinate if both got a number below some intege (the consecutive number game \ \citet{van1980conway,van2015one}). We decided against this exactly for reasons of ecological validity, since the abstraction made the game unnecessarily opaque. TODO: Expand this?

Second, we can ask if situations arise in the wild, where there is only shared knowledge to some degree, but not common knowledge. It is relevant here to notice that the essential problem in both the mountaineering example and our canteen dilemma, is the lack of reliable signaling. And unlike those cases, we can perhaps in most real cases assume that our signal or message (both face-to-face and online) has reached the recipient.  But, while the signal itself might reliably reach them, this is not sufficient for establishing common knowledge about it. They must also reliably interpret it \emph{and} this reliableness must be common knowledge.  So, depending on the context and complexity of the intentionality involved, I might not be sure that my communicated intention becomes common knowledge. Note that this is because that humans do not just lack logical omniscience, often prescribed in logical analysis, but also perfect observation and recall. That is, if we make an announcement of something, we cannot be certain that everyone then knows this, or that they will maintain it as knowledge. Public broadcasting of messages, e.g.\ instructions given in a classroom to a class of students or an email sent by the department head to everybody in a university department, will also in practice not necessarily lead to common knowledge. Some of the students might not be paying attention at the moment, and some of the department employees might not have checked their email, or it might have ended up in their spam filter. Suppose that the message was regarding to all meet at a certain place at a certain time. Can we then be sure that everyone will meet at that place at that time? Obviously, no. Then the reply might be that we can just ask everybody to confirm that they received the message. Well, yes, but then the same argument repeats for the confirmation messages: the recipient of the confirmations might not receive all of them or might briefly not pay attention. Now, this generalizes to all social situations and while it does not mean that we don't often have common knowledge, it means it is possible that we might end up in situations where we have shared knowledge to some depth, but not common knowledge. In most everyday sitatuons, the penalty for confusing shared knowledge with common knowledge might also be much lower than in our game. However, this should lead humans to pay more attention to it in our game than in the wild. In other words, if participants do not pay attention to it in our game, we should not expect them to be more aware of it in the wild.
We might indeed have evolved to do our best to avoid such situations, due to the complexity involved. This might explain why we do not identity it everywhere in our daily life. But it is unlikely that we can categorically deny that such situations arise, and when they do, some might take it for common knowledge, while others, on a lower order of shared knowledge might not. This results in miscoordination which people cannot predict, and possibly not even post-hoc understand. 
